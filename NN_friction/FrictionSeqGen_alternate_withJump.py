...
def O_parallel(fs, f_targs, ts, p = 2, pool = parallel_pool):
    ...
    return torch.pow(torch.mean(torch.stack(Os)), 1. / p)
...
def grad_parallel(beta, ts, Vs, thetas, fs, f_targs, t_JumpIdxs, tts, VVs, JumpIdxs, kwgs, p = 2, pool = parallel_pool):
    grads = pool(
        delayed(grad)(beta, t, V, theta, f, f_targ, t_JumpIdx, tt, VV, JumpIdx, p, kwgs) \
...
def grad(beta, t, V, theta, f, f_targ, t_JumpIdx, tt, VV, JumpIdx, p, kwgs):
...
pwd = "./plots/Test0601_std_0_AdjMtd_generated_intervals/"
...
# O_this = 0.
# grad_this = torch.zeros(4)
# O_this = O_parallel(f_thiss, f_targs, ts, parallel_pool)
# grad_this = grad_parallel(beta_this, ts, V_thiss, theta_thiss, f_thiss, f_targs, t_JumpIdxs, tts, VVs, JumpIdxs, kwgs, parallel_pool)

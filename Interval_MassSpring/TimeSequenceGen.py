...
class TimeSequenceGen:
    ...
    def __init__(self, NofTPts, MFParams, rtol = 1.e-6, atol = 1.e-8, regularizedFlag = True, solver = 'dopri5'):
        ...
        self.t = torch.linspace(0., self.T, NofTPts * len(MFParams.TT))
        ...
        # Calculate self.V
        self.V = torch.zeros(self.t.shape)
        for i in range(len(self.t_JumpIdx) - 1):
            self.V[self.t_JumpIdx[i] : self.t_JumpIdx[i + 1]] = torch.tensor(self.MFParams.VtFuncs[i](
                    self.t[self.t_JumpIdx[i] : self.t_JumpIdx[i + 1]]
                ), dtype=torch.float
            )
